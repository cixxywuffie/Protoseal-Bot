...
public class GlobalCommandRegistrar implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws IOException {
        ...
        */
        var registeredCommands = applicationService
                .bulkOverwriteGlobalApplicationCommand(applicationId, commands)
                .collectList()
                .block();

        int registeredCount = registeredCommands == null ? 0 : registeredCommands.size();
        LOGGER.info("Registered {} global commands: {}", registeredCount,
                commands.stream().map(ApplicationCommandRequest::name).sorted().toList());
    }
}
